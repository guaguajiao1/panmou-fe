    ...
    <view class="scroll-content">
      ...
      <view class="purchase-options">
        ...
        <view class="option-item" @click="purchaseType = 'onetime'">
          ...
          <view class="option-details">
            <view class="detail-line">
              <text class="buy-once-title">买一次</text>
            </view>
            <view class="detail-line">
              <text class="price-large"
                >￥{{ selectedSkuPrice > 0 ? selectedSkuPrice.toFixed(2) : '--' }}</text
              >
            </view>
            <view class="detail-line" v-if="savings > 0">
              <text class="discount-info">订阅可省￥{{ savings.toFixed(2) }} (省10%)</text>
            </view>
          </view>
        </view>
...
const savings = computed(() => {
  const oneTime = selectedSkuPrice.value
  // subscriptionPrice 是字符串，需要转换
  const sub = parseFloat(subscriptionPrice.value)
  if (oneTime > 0 && sub > 0 && oneTime > sub) {
    return oneTime - sub
  }
  return 0
})
